...
export default function ClientHeader() {
  ...
  const goToProfile = () => navigate(`/${base}/profile`);

  useEffect(() => {
    const handleClickOutside = (e) => {
      // only close dropdown, don't block events
      setShowNotif(false);
    };
  ...
  useEffect(() => {
    // 🔥 LOAD TOP 6
    api.get("/notifications/client/top").then((data) => {
      setNotifications(Array.isArray(data) ? data : []);
    });

    // 🔥 SOCKET
    connectSocket(orgId, role, (msg) => {
        ...
        >
          ...
          {showNotif && (
            <div className="dropdown notif-dropdown">
              ...
              <button onClick={() => navigate(`/${base}/notifications`)}>
                See More →
              </button>
            </div>
          )}
