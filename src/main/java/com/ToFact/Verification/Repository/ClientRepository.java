...
////	MySQL
	@Query("""
			...
			""")
	List<Client> searchClients(@Param("q") String q, @Param("location") String location, @Param("size") String size);
...
//		        :location IS NULL
//		        OR :location = ''
//		        OR LOWER(c.location) LIKE LOWER(CONCAT('%', :location, '%'))
//		    )
//		    AND ( :size IS NULL OR :size = ''  OR c.employeeCount = :size ) ORDER BY c.createdAt DESC """)
//	List<Client> searchClients(@Param("q") String q, @Param("location") String location, @Param("size") String size);
